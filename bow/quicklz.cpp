...
int QuickLZ::same(const unsigned char *src, size_t n)
{
...
void QuickLZ::reset_table_compress(QlzStateCompress* state)
{
...
void QuickLZ::reset_table_decompress(QlzStateDecompress* state)
{
	int i;
	(void)state;
	(void)i;
...
unsigned int QuickLZ::hash_func(unsigned int i)
{
#if QLZ_COMPRESSION_LEVEL == 2
...
unsigned int QuickLZ::fast_read(void const* src,unsigned int bytes)
{
#ifndef X86X64
...
unsigned int QuickLZ::hashat(const unsigned char* src)
{
...
void QuickLZ::fast_write(unsigned int f,void* dst,size_t bytes)
{
#ifndef X86X64
...
size_t QuickLZ::qlz_size_decompressed(const char* source)
{
	unsigned int n, r;
	n = (((*source) & 2) == 2) ? 4 : 1;
...
size_t QuickLZ::qlz_size_compressed(const char* source)
{
	unsigned int n, r;
	n = (((*source) & 2) == 2) ? 4 : 1;
...
size_t QuickLZ::qlz_size_header(const char* source)
{
	size_t n = 2*((((*source) & 2) == 2) ? 4 : 1) + 1;
	return n;
}

void QuickLZ::memcpy_up(unsigned char* dst,const unsigned char* src,unsigned int n)
{
...
void QuickLZ::update_hash(QlzStateDecompress* state,const unsigned char* s)
{
#if QLZ_COMPRESSION_LEVEL == 1
	unsigned int hash;
	hash = hashat(s);
...
#if QLZ_COMPRESSION_LEVEL <= 2
void QuickLZ::update_hash_upto(QlzStateDecompress* state,unsigned char** lh,const unsigned char* max)
{
	while(*lh < max){
		(*lh)++;
...
size_t QuickLZ::qlz_compress_core(const unsigned char* source,unsigned char* destination,size_t size,QlzStateCompress* state)
{
	...
	while(src <= last_matchstart){
...
#elif QLZ_COMPRESSION_LEVEL >= 2
		{
			...
			if(matchlen > 2 && src - o < 131071){
				...
				if(matchlen == 3 && offset <= 63){
					*dst = (unsigned char)(offset << 2);
					...
					unsigned int f = (unsigned int)((offset << 2) | 1);
					fast_write(f, dst, 2);
					dst += 2;
				}		
				else if(matchlen <= 18 && offset <= 1023){
					unsigned int f = ((matchlen - 3) << 2) | ((unsigned int)offset << 6) | 2;
					fast_write(f,dst,2);
					dst += 2;
				}
				else if(matchlen <= 33){
					unsigned int f = ((matchlen - 2) << 2) | ((unsigned int)offset << 7) | 3;
					fast_write(f,dst,3);
					dst += 3;
				}
				else{
					unsigned int f = ((matchlen - 3) << 7) | ((unsigned int)offset << 15) | 3;
					fast_write(f,dst,4);
					dst += 4;
				}
			}
			...
			if(matchlen > 2){
				...
				if(matchlen < 10){			
					unsigned int f = best_k | ((matchlen - 2) << 2) | (hash << 5);
					fast_write(f,dst,2);
					dst += 2;
				}
				else{
					unsigned int f = best_k | (matchlen << 16) | (hash << 5);
					fast_write(f,dst,3);
					dst += 3;
				}
			}
...
size_t QuickLZ::qlz_decompress_core(const unsigned char* source,unsigned char* destination,size_t size,QlzStateDecompress* state,const unsigned char* history)
{
	...
	for(;;){
		...
		if((cword_val & 1) == 1){
			...
			if(matchlen > (unsigned int)(last_destination_byte - dst - UNCOMPRESSED_END + 1)) return 0;
#endif
...
size_t QuickLZ::qlz_compress(const void* source,char* destination,size_t size,QlzStateCompress* state)
{
	size_t r;
...
size_t QuickLZ::qlz_decompress(const char* source,void* destination,QlzStateDecompress* state)
{
