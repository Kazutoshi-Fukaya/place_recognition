...
public:
    ...
    void inpaint_img(cv::Mat input_img,cv::Mat& output_img)
    {
        ...
        cv::rectangle(mask_img,cv::Point(pillars_.fourth.start,0),cv::Point(pillars_.fourth.end,input_img.rows),cv::Scalar(255,255,255),-1,cv::LINE_AA);        cv::cvtColor(mask_img,mask_img,cv::COLOR_BGR2GRAY);
