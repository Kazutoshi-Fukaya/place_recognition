    ...
    while(std::getline(ifs,line)){
        ...
        try{
			...
			if(image_mode == "rgb"){
				...
				std::cerr << "No applicable 'image_mode'. Please select 'rgb' or 'equ'" << std::endl;
				return;
			}
...
void MultiPlaceRecognition::create_database(std::string reference_images_path,std::string image_mode)
{
	...
	std::string database_file = reference_images_path + image_mode + "/dkan_mono.yml.gz";
...
void MultiPlaceRecognition::publish_result()
{
	cv::Mat img = interfaces_->get_img();
